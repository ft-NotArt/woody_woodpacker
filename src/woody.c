...
//see man elf, ident must contain 0x7fELF
#define IS_ELF(ident) (ident[0] != ELFMAG0 || ident[1] != ELFMAG1 || ident[2] != ELFMAG2 || ident[3] != ELFMAG3)

size_t new_stub_len;


bool check_file(int fd, t_elf *elf) {
	struct stat st;
	if (fstat(fd, &st) < 0) {
		perror("fstat");
		return false;
...
bool check_elf(const char *path, t_elf *elf, int *out_fd) {
...
unsigned char *generate_key(size_t *key_size) {
	...
	key[0] = first;
	if (len > 1 && read(fd, key + 1, len - 1) != (ssize_t) len - 1) {
		perror("read /dev/urandom");
		close(fd);
		free(key);
		return NULL;
...
bool parse_args(int ac, char **av, unsigned char **out_key, size_t *key_size) {
	if (!(ac == 2 || ac == 3)) {
...
int build_woody(t_elf *elf, Elf64_Phdr *exe_seg, Elf64_Addr encrypted_vaddr, Elf64_Addr original_entry, unsigned char *encrypt_buff __attribute__((unused)), size_t encrypt_size, unsigned char *key, size_t key_size)
{
	...
	// Calculate stub_vaddr with correct alignment
	...
	int offset = exe_seg->p_offset == 0 ? 0x1000 : exe_seg->p_offset; // Skip one page when offset is zero
